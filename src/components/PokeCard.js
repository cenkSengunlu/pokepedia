  ...
  return (
    <>
      {pokemon && pokeCardTitle && typeColor &&
        <>
          <NavLink to={`/pokemon/${pokemon.pokemon.name}`} className={`bg-zinc-200 shadow-xl text-zinc-500 rounded-2xl py-5 px-6 cursor-pointer select-none w-48 ${typeColor} `} onClick={() => setPokeName(pokemon.pokemon.name)}>
